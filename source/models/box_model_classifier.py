...
class BoxModelClassifier(TransformerBoxModel):
    ...
    def classify(self, mention_context_list):
        ...
        probs = self.classifier(mention_context_rep).detach().numpy()
        return [[self.types[j] for j in range(len(self.types)) if probs[i][j] > TYPE_SIMILARITY_THRESHOLD] for i in range(len(probs))]
